...
# ─────────────────────────────────────────────────────────────────────────────
# CAMPAIGN DATA — v3 (sender introduction fix)
# ─────────────────────────────────────────────────────────────────────────────
...
DAMIAN_HUCZ = {
    ...
    "e1_body": (
        ...
        "Nazywam się Tomasz Uściński, jestem z polskiej firmy Profitia, w której od 15 lat pomagamy "
        "dyrektorom zakupów w firmach dystrybucyjnych wypracować wspólną logikę oceny ofert i podwyżek, "
        "tak żeby wynik rozmów z dostawcami mniej zależał od tego, kto akurat prowadzi daną kategorię.\n\n"
...
MONIKA_SITKOWSKA = {
    ...
    "e1_body": (
        ...
        "Nazywam się Tomasz Uściński, jestem z polskiej firmy Profitia, w której od 15 lat pomagamy "
        "dyrektorom zakupów w sieciach handlowych szybciej oddzielać podwyżki realnie wynikające "
        "ze zmian kosztowych od tych, z którymi warto wrócić do rozmowy.\n\n"
...
def build_campaign_doc(data, output_path):
    ...
    add_run(p2, "Wersja: v3 (sender introduction fix)", size=10, color="9CA3AF")
...
def build_comparison_doc(output_path):
    ...
    add_title(doc, "Porównanie kampanii outbound - v3 (sender introduction fix)")
    p = doc.add_paragraph()
    set_paragraph_spacing(p, before_pt=0, after_pt=2)
    ...
    add_section_header(doc, "3. Co się zmieniło vs v2")

    add_subsection_header(doc, "Damian Hucz - zmiany w v3")
    add_body_text(
        doc,
        "- Przedstawienie nadawcy: \"Jestem Tomasz Uściński z Profitii\" zastąpione "
        "\"Nazywam się Tomasz Uściński, jestem z polskiej firmy Profitia, w której od 15 lat pomagamy...\". "
        "- Naturalna forma polska (\"Nazywam się\" zamiast \"Jestem\"). "
        "- Kontekst marki od razu: polska firma, 15 lat doświadczenia. "
        "- Reszta kampanii bez zmian - zachowany humanized tone z v2."
    )

    add_subsection_header(doc, "Monika Sitkowska - zmiany w v3")
    add_body_text(
        doc,
        "- Przedstawienie nadawcy: \"Jestem Tomasz Uściński z Profitii\" zastąpione "
        "\"Nazywam się Tomasz Uściński, jestem z polskiej firmy Profitia, w której od 15 lat pomagamy...\". "
        "- Identyczna zmiana logiki jak u Hucza, ale część zmienna dopasowana do kontekstu "
        "ALDI (sieci handlowe, weryfikacja podwyżek). "
        "- Reszta kampanii bez zmian - zachowany humanized tone z v2."
    )
    ...
    add_body_text(
        doc,
        "v3 wprowadza jedną trwałą zmianę: naturalniejszy framework przedstawienia nadawcy. "
        "Zamiast nienaturalnego \"Jestem Tomasz Uściński z Profitii\" - \"Nazywam się Tomasz Uściński, "
        "jestem z polskiej firmy Profitia, w której od 15 lat pomagamy...\". "
        "Reszta kampanii (hipotezy, follow-upy, CTA, ton) zachowana z v2 bez zmian."
    )
...
def main():
    ...
    print("Generuję kampanie outbound v3 (sender introduction fix)...\n")

    path1 = os.path.join(output_dir, "Damian_Hucz_campaign_v3.docx")
    build_campaign_doc(DAMIAN_HUCZ, path1)

    path2 = os.path.join(output_dir, "Monika_Sitkowska_campaign_v3.docx")
    build_campaign_doc(MONIKA_SITKOWSKA, path2)

    path3 = os.path.join(output_dir, "campaign_summary_comparison_v3.docx")
